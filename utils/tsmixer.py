...
class TSMixerConf(DataClassDictMixin):
    ...
    def load_training_metadata_or_new(self, epoch_start: Optional[int] = None) -> "TSMixer.TrainingMetadata":
        ...
        if os.path.exists(self.train_progress_json):
            with open(self.train_progress_json, "r") as f:
                tp = TSMixer.TrainingMetadata.from_dict(json.load(f))
            ...
            return TSMixer.TrainingMetadata(epoch_to_data={})
    ...
    def write_training_metadata(self, train_data: "TSMixer.TrainingMetadata"):
